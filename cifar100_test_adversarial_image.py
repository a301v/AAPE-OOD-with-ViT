...
test_dataset = CIFAR100(root='/Users/unicorn/Desktop/OOD-with-ViT-main/data', train=False, download=False, transform=transform)
...
os.makedirs('cifar100_test_adversarial/attention_map', exist_ok=True)

# Create output directory for attention rollouts
os.makedirs('cifar100_test_adversarial/attention_rollout', exist_ok=True)
...
for i, (input_image, target) in enumerate(test_loader):
    ...
    attn_map_output_path = f"cifar100_test_adversarial/attention_map/attention_map_{i}.png"
    attn_rollout_output_path = f"cifar100_test_adversarial/attention_rollout/attention_rollout_{i}.npy"
    ...
    attn_rollout_output_path = f"cifar100_test_adversarial/attention_rollout/attention_rollout_{i}"
...
os.makedirs('cifar100_test_adversarial/attention_mask', exist_ok=True)

# Create output directory for perturbed image
os.makedirs('cifar100_test_adversarial/perturbed_image_numpy', exist_ok=True)

# Create output directory for attention adversarial image
os.makedirs('cifar100_test_adversarial/attention_adversarial_image_numpy', exist_ok=True)


os.makedirs('cifar100_test_adversarial/perturbed_image', exist_ok=True)
os.makedirs('cifar100_test_adversarial/attention_adversarial_image', exist_ok=True)
...
for i, (image_tensor, target) in enumerate(test_loader):
    ...
    attn_rollout = np.load(f"cifar100_test_adversarial/attention_rollout/attention_rollout_{i}.npy")
    attention_mask = image_maker.get_attention_mask(image_tensor, attn_rollout, 0.5)
    perturbed_image_tensor = image_maker.reversed_fgsm_attack(image_tensor, pseudo_target, epsilon = -0.01, temperature = 1)
    ...
    attn_mask_path = f"cifar100_test_adversarial/attention_mask/attention_mask_{i}"
    np.save(attn_mask_path, attention_mask)

    # Save the perturbed image numpy data
    perturbed_numpy_path = f"cifar100_test_adversarial/perturbed_image_numpy/perturbed_image_numpy_{i}"
    np.save(perturbed_numpy_path, perturbed_image)

    # Save the attention adversarial image numpy data
    attn_adv_numpy_path = f"cifar100_test_adversarial/attention_adversarial_image_numpy/attention_adversarial_image_numpy_{i}"
    ...
    perturbed_path = f"cifar100_test_adversarial/perturbed_image/perturbed_image_{i}.jpg"
    cv2.imwrite(perturbed_path, denormalized_perturbed_image_rgb)


    # Save the attention adversarial image data
    attn_adv_path = f"cifar100_test_adversarial/attention_adversarial_image/attention_adversarial_image_{i}.jpg"
