...
os.makedirs('cifar10_train_adversarial/attention_map', exist_ok=True)

# Create output directory for attention rollouts
os.makedirs('cifar10_train_adversarial/attention_rollout', exist_ok=True)
...
for i, (input_image, target) in enumerate(test_loader):
    ...
    attn_map_output_path = f"cifar10_train_adversarial/attention_map/attention_map_{i}.png"
    cv2.imwrite(attn_map_output_path, visualized_map)

    # Save the attention rollout numpy data
    attn_rollout_output_path = f"cifar10_train_adversarial/attention_rollout/attention_rollout_{i}"
...
class MakeAttentionAdversarialImage:
    ...
    def get_attention_mask(self, image_tensor, attn_rollout, perturb_ratio = 0.7):
...
os.makedirs('cifar10_train_adversarial/attention_mask', exist_ok=True)

# Create output directory for perturbed image
os.makedirs('cifar10_train_adversarial/perturbed_image_numpy', exist_ok=True)

# Create output directory for attention adversarial image
os.makedirs('cifar10_train_adversarial/attention_adversarial_image_numpy', exist_ok=True)


os.makedirs('cifar10_train_adversarial/perturbed_image', exist_ok=True)
os.makedirs('cifar10_train_adversarial/attention_adversarial_image', exist_ok=True)
...
for i, (image_tensor, target) in enumerate(test_loader):
    ...
    attn_rollout = np.load(f"cifar10_train_adversarial/attention_rollout/attention_rollout_{i}.npy")
    attention_mask = image_maker.get_attention_mask(image_tensor, attn_rollout, 1)
    perturbed_image_tensor = image_maker.reversed_fgsm_attack(image_tensor, target, epsilon = 0.001, temperature = 1000)
    ...
    attn_mask_path = f"cifar10_train_adversarial/attention_mask/attention_mask_{i}"
    np.save(attn_mask_path, attention_mask)

    # Save the perturbed image numpy data
    perturbed_numpy_path = f"cifar10_train_adversarial/perturbed_image_numpy/perturbed_image_numpy_{i}"
    np.save(perturbed_numpy_path, perturbed_image)

    # Save the attention adversarial image numpy data
    attn_adv_numpy_path = f"cifar10_train_adversarial/attention_adversarial_image_numpy/attention_adversarial_image_numpy_{i}"
    ...
    perturbed_path = f"cifar10_train_adversarial/perturbed_image/perturbed_image_{i}.jpg"
    cv2.imwrite(perturbed_path, denormalized_perturbed_image_rgb)


    # Save the attention adversarial image data
    attn_adv_path = f"cifar10_train_adversarial/attention_adversarial_image/attention_adversarial_image_{i}.jpg"
